...
async function upsertLink(supabase: ServiceClient, candidate: LinkCandidate) {
  let lookup = supabase
    ...
    .select("id, confidence, status, match_method")
  ...
  if (identifiers.orderNumbers.length) {
    ...
    for (const order of orders as Array<{ id: string; order_number: string }>) {
      candidates.push({
        ...
        confidence: 1.0,
        status: "confirmed",
        metadata: linkMetadata(identifiers, "order_number_exact"),
      });
    }
  }
  ...
  if (identifiers.itemNumbers.length && identifiers.transactionIds.length) {
    ...
    for (const line of lines as Array<Record<string, any>>) {
      ...
      candidates.push({
        ...
        confidence: 1.0,
        status: "confirmed",
        metadata: linkMetadata(identifiers, "item_transaction_exact"),
      });
    }
  }
    ...
    for (const [itemNumber, matchedLines] of byItem) {
      ...
      if (matchedLines.length === 1) {
        ...
        candidates.push({
          ...
          confidence: 0.8,
          status: "suggested",
          metadata: linkMetadata(identifiers, "item_number_exact"),
        });
    ...
    for (const label of labelCandidates.slice(0, 10)) {
      ...
      if (lineTitleMatches.length === 1) {
        ...
        candidates.push({
          ...
          confidence: 0.55,
          status: "suggested",
          metadata: linkMetadata(identifiers, "internal_label_item_title_contains"),
        });
      ...
      if (itemTitleMatches.length === 1) {
        ...
        candidates.push({
          ...
          confidence: 0.5,
          status: "suggested",
          metadata: linkMetadata(identifiers, "internal_label_inventory_title_contains"),
        });
