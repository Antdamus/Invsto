-- Allow eBay conversation message tasks to be created with photo evidence.
-- Forward-only migration because earlier migration files may already be applied.
...
drop function if exists public.create_ebay_conversation_message_task(uuid, uuid, text, text, uuid, text, timestamptz, text, numeric, jsonb, text);
...
declare
  ...
  v_metadata jsonb;
begin
  if not public.can_access_email_triage() then
...
revoke all on function public.create_ebay_conversation_message_task(uuid, uuid, text, text, uuid, text, timestamptz, text, numeric, jsonb, text) from public;
grant execute on function public.create_ebay_conversation_message_task(uuid, uuid, text, text, uuid, text, timestamptz, text, numeric, jsonb, text) to authenticated;

comment on function public.create_ebay_conversation_message_task(uuid, uuid, text, text, uuid, text, timestamptz, text, numeric, jsonb, text)
  is 'Creates an audited customer-service team task from one canonical eBay conversation message, with optional refund tag metadata and photo evidence.';
