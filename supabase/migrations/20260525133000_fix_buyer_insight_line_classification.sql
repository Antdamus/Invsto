-- Keep Buyer Insight line buckets independent from the page that opened the modal.
-- Skipped/archive lines must not be counted as current pending just because the
-- parent order is still pending.
...
begin
  ...
  line_order_totals as (
    ...
    from raw_lines rl
    where coalesce(rl.line_status, '') <> 'skipped'
  ...
  line_rows as (
    select
      ...
      case
        ...
        when rabo.order_number is not null then 'return'
      ...
      case
        when rabo.order_number is not null and coalesce(rl.line_status, '') <> 'skipped' and lot.line_total_sum > 0
          then round(rabo.returned_amount * (rl.line_total / lot.line_total_sum), 2)
        when rabo.order_number is not null and coalesce(rl.line_status, '') <> 'skipped'
