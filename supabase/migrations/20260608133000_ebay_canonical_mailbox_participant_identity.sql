-- Step 5F.6P.2 canonical eBay mailbox participant identity repair.
-- Replaces the v2 RPC so mailbox rows include message-derived participant
-- usernames for display fallback. Read-only; no sync, send, or eBay mutation.
...
begin
  ...
  with base as materialized (
    select
      ...
      coalesce(message_stats.participant_usernames, '{}'::text[]) as participant_usernames,
      message_stats.message_buyer_username,
      lower(concat_ws(
        ' ',
        c.id::text,
    ...
    left join lateral (
      ...
      participant_rows as (
        select nullif(btrim(username), '') as username
        from message_rows m
        cross join lateral (values (m.sender_username), (m.recipient_username)) as participant(username)
      ),
      buyer_candidates as (
        select
          nullif(btrim(case
            when lower(coalesce(m.direction, '')) = 'outbound' then m.recipient_username
      ...
      select
        ...
        coalesce((
          select array_agg(distinct p.username order by p.username)
          from participant_rows p
          where p.username is not null
            and lower(p.username) <> lower(coalesce(s.seller_username, ''))
            and lower(p.username) <> 'ebay'
        ), '{}'::text[]) as participant_usernames,
        (
          select bc.username
          from buyer_candidates bc
          where bc.username is not null
            and lower(bc.username) <> lower(coalesce(s.seller_username, ''))
            and lower(bc.username) <> 'ebay'
          order by bc.created_at_ebay desc nulls last, bc.created_at desc
          limit 1
        ) as message_buyer_username,
        string_agg(concat_ws(
          ' ',
          m.ebay_message_id,
          m.sender_username,
      ...
      select jsonb_agg(
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'summary', jsonb_build_object(
            ...
            'message_buyer_username', p.message_buyer_username,
            'links', p.link_rows
          ),
...
comment on function public.get_ebay_canonical_mailbox_v2(integer, integer, text, text[], jsonb, jsonb)
  is 'Versioned canonical eBay mailbox read model with provider/local read state and participant username fallback for Step 5F.6P.2. Read-only; derives source from ebay_conversations.conversation_type.';
