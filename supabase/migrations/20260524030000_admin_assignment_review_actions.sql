-- Admin controls for employee reassignment requests and assignment cleanup.
...
begin
  ...
  if v_source = 'team' then
    ...
    set assigned_to_user_id = case
          when v_action = 'cancel_assignment' then null
          when v_action = 'reassign' then v_employee.user_id
          else assigned_to_user_id
        end,
        assigned_to_employee_id = case
          when v_action = 'cancel_assignment' then null
          when v_action = 'reassign' then v_employee.id
          else assigned_to_employee_id
        end,
        assigned_to_email = case
          when v_action = 'cancel_assignment' then null
          when v_action = 'reassign' then v_employee.email
          else assigned_to_email
        end,
        assigned_to_role = case
          when v_action = 'cancel_assignment' then null
          when v_action = 'reassign' then v_employee.role
    ...
    v_event_action := case
      when v_action in ('reassign', 'cancel_assignment') then 'assigned'
      else 'commented'
    end;
    ...
    values (
      ...
      v_old_assigned, v_team_task.assigned_to_user_id, v_note,
