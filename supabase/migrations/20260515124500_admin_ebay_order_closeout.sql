-- Admin-only closeout for pending eBay orders when no inventory row is removed.
-- Used for orders shipped outside the inventory system or cancelled on eBay.

create table if not exists public.ebay_order_admin_events (
  id uuid primary key default gen_random_uuid(),
  action text not null check (action in ('fulfilled_no_inventory', 'cancelled')),
...
alter table public.ebay_order_admin_events enable row level security;
drop policy if exists "ebay_order_admin_events_admin_select" on public.ebay_order_admin_events;
create policy "ebay_order_admin_events_admin_select"
...
create policy "ebay_order_admin_events_admin_insert"
on public.ebay_order_admin_events
for insert
to authenticated
with check (public.is_admin());
grant select, insert on table public.ebay_order_admin_events to authenticated;
...
begin
  if not public.is_admin() then
    raise exception 'Only admins can close pending eBay orders without inventory removal' using errcode = '42501';
  end if;
  ...
  if v_action not in ('fulfilled_no_inventory', 'cancelled') then
    raise exception 'Invalid admin closeout action' using errcode = '22023';
  end if;

  if v_note is null then
    raise exception 'A note is required for admin order closeout' using errcode = '22023';
  end if;
