-- Step 5F.6A.1C canonical eBay mailbox read model replacement.
-- Versioned and read-only: this does not alter the existing frontend path,
-- mutate eBay, or mutate local read/unread state.
...
comment on function public.get_ebay_canonical_mailbox_v2(integer, integer, text, text[], jsonb, jsonb)
  is 'Versioned canonical eBay mailbox read model for Step 5F.6A.1C validation. Read-only; derives source from ebay_conversations.conversation_type.';
