-- Expose cancellation value separately in buyer insights.
-- Prior orders stay limited to real completed purchases: returned and
-- cancelled orders explain the history without counting as prior value.
...
begin
  ...
  returned_order_numbers as (
    select distinct rb.order_number
    from return_base rb
  ...
  order_summary as (
    select
      ...
      count(*) filter (where bo.status not in ('cancelled', 'archived')) as gross_order_count,
      count(*) filter (
        where bo.status not in ('cancelled', 'archived')
          and not exists (
            select 1
            from returned_order_numbers ron
            where ron.order_number = bo.order_number
          )
      ) as order_count,
      ...
      round(coalesce(sum(bo.total_price) filter (where bo.status = 'cancelled'), 0), 2) as cancelled_order_total,
      round(coalesce(sum(bo.net_payout) filter (where bo.status = 'cancelled'), 0), 2) as cancelled_net_payout_total,
      round(coalesce(sum(bo.total_price) filter (where bo.status not in ('cancelled', 'archived')), 0), 2) as gross_sales_before_returns,
      round(coalesce(sum(bo.net_payout) filter (where bo.status not in ('cancelled', 'archived')), 0), 2) as net_payout_before_returns,
      round(coalesce(sum(bo.shipping_and_handling) filter (where bo.status not in ('cancelled', 'archived')), 0), 2) as shipping_total,
      min(bo.purchase_at) filter (
        where bo.status not in ('cancelled', 'archived')
          and not exists (
            select 1
            from returned_order_numbers ron
  ...
  line_summary as (
    select
      count(*) filter (
        where lb.line_status not in ('cancelled', 'skipped')
          and not exists (
            select 1
            from returned_order_numbers ron
            where ron.order_number = lb.order_number
          )
      ) as line_count,
      coalesce(sum(lb.quantity) filter (
        where lb.line_status not in ('cancelled', 'skipped')
          and not exists (
            select 1
            from returned_order_numbers ron
      ...
      round(coalesce(sum(lb.total_price) filter (where lb.line_status = 'cancelled'), 0), 2) as cancelled_line_total
    from line_base lb
  ),
  return_summary as (
    select
      count(*) filter (where lower(coalesce(rb.status, '')) <> 'cancelled') as return_count,
      count(*) filter (where lower(coalesce(rb.status, '')) not in ('closed', 'cancelled')) as open_return_count,
      round(coalesce(sum(rb.request_amount_value) filter (where lower(coalesce(rb.status, '')) <> 'cancelled'), 0), 2) as return_amount_total,
      round(coalesce(sum(rb.request_amount_value) filter (where lower(coalesce(rb.status, '')) not in ('closed', 'cancelled')), 0), 2) as open_return_amount_total
    from return_base rb
  ),
  net_summary as (
    select
      os.*,
      greatest(0::numeric, round(coalesce(os.gross_sales_before_returns, 0) - coalesce(rs.return_amount_total, 0), 2)) as gross_sales,
      greatest(0::numeric, round(coalesce(os.net_payout_before_returns, 0) - coalesce(rs.return_amount_total, 0), 2)) as net_payout
  ...
  recent_cancelled_order_rows as (
    select
      bo.id,
      bo.order_number,
      bo.status,
  ...
  select jsonb_build_object(
    'buyerUsername', ns.buyer_username,
    'buyerName', ns.buyer_name,
    'summary', jsonb_build_object(
      'orderCount', coalesce(ns.order_count, 0),
      'grossOrderCount', coalesce(ns.gross_order_count, 0),
      'pendingOrderCount', coalesce(ns.pending_order_count, 0),
      'fulfilledOrderCount', coalesce(ns.fulfilled_order_count, 0),
      'cancelledOrderCount', coalesce(ns.cancelled_order_count, 0),
      'cancelledOrderTotal', coalesce(ns.cancelled_order_total, 0),
      'cancelledNetPayoutTotal', coalesce(ns.cancelled_net_payout_total, 0),
      ...
      'cancelledLineTotal', coalesce(ls.cancelled_line_total, 0),
      'grossSales', coalesce(ns.gross_sales, 0),
      'grossSalesBeforeReturns', coalesce(ns.gross_sales_before_returns, 0),
      'netPayout', coalesce(ns.net_payout, 0),
      'netPayoutBeforeReturns', coalesce(ns.net_payout_before_returns, 0),
      'shippingTotal', coalesce(ns.shipping_total, 0),
      'avgOrderValue', round(coalesce(ns.gross_sales, 0) / nullif(ns.order_count, 0), 2),
      'returnCount', coalesce(rs.return_count, 0),
      'openReturnCount', coalesce(rs.open_return_count, 0),
      'returnAmountTotal', coalesce(rs.return_amount_total, 0),
      'openReturnAmountTotal', coalesce(rs.open_return_amount_total, 0),
      'returnRate', round(coalesce(rs.return_count, 0)::numeric / nullif(ns.gross_order_count, 0), 4),
      'firstPurchaseAt', ns.first_purchase_at,
      'lastPurchaseAt', ns.last_purchase_at
    ),
    'itemKinds', coalesce((
    ...
    'recentCancellations', coalesce((
      select jsonb_agg(jsonb_build_object(
        'orderNumber', rcor.order_number,
        'status', rcor.status,
        'purchaseAt', rcor.purchase_at,
        'totalPrice', rcor.total_price,
        'netPayout', rcor.net_payout,
        'lineCount', rcor.line_count,
        'unitCount', rcor.unit_count,
        'itemTitles', rcor.item_titles
      ) order by rcor.purchase_at desc nulls last)
      from recent_cancelled_order_rows rcor
    ), '[]'::jsonb)
  )
  into v_result
  from net_summary ns
  ...
  return coalesce(v_result, jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'unitCount', 0,
      'grossSales', 0,
      'grossSalesBeforeReturns', 0,
      'netPayout', 0,
      'netPayoutBeforeReturns', 0,
      'returnCount', 0,
      'openReturnCount', 0,
      'returnAmountTotal', 0,
      'openReturnAmountTotal', 0,
      'cancelledOrderCount', 0,
      'cancelledOrderTotal', 0,
      'cancelledNetPayoutTotal', 0,
      'cancelledLineCount', 0,
      'cancelledLineTotal', 0
    ),
    'itemKinds', '[]'::jsonb,
    'topItems', '[]'::jsonb,
    'recentOrders', '[]'::jsonb,
    'recentReturns', '[]'::jsonb,
    'recentCancellations', '[]'::jsonb
  ));
end;
$$;
