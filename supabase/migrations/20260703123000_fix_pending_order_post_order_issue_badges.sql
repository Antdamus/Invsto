...
begin
  ...
  left join lateral (
    ...
    select
      ...
      (array_agg(case when rc.issue_text ~ '(dispute|escalat|case)' then 'dispute' else 'return_request' end order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_type,
      (array_agg(case when rc.issue_text ~ '(dispute|escalat|case)' then 'Dispute' else 'Return request' end order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_label,
      (array_agg(coalesce(nullif(rc.raw_payload->>'returnStatus', ''), nullif(rc.raw_payload->>'returnState', ''), nullif(rc.raw_payload->>'returnAction', ''), nullif(rc.status, '')) order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_status,
      (array_agg(coalesce(nullif(rc.return_reason, ''), nullif(rc.raw_payload->>'returnReason', ''), nullif(rc.raw_payload #>> '{returnDetails,buyerComment}', '')) order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_reason,
      max(rc.opened_at) as issue_latest_at,
      (array_agg(case when rc.line_match then 'line' else 'order' end order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_scope,
      (array_agg(coalesce(nullif(rc.raw_payload #>> '{returnDetails,detailsUrl}', ''), nullif(rc.raw_payload->>'detailsUrl', '')) order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_url,
      (array_agg(jsonb_build_object(
        ...
        'scope', case when rc.line_match then 'line' else 'order' end,
      ...
      ) order by rc.line_match desc, rc.opened_at desc, rc.id desc))[1] as issue_payload
