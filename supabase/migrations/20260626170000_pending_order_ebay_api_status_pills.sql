-- Expose compact eBay API status fields to the fast pending-order queue.
-- Keep the raw eBay payload out of first paint; only small normalized strings
-- are returned so the UI can show cancellation/refund/fulfillment warnings.
...
comment on function public.list_pending_ebay_order_queue(text, boolean, integer, integer)
  is 'Fast pending eBay order queue read path with compact eBay API status fields for cancellation/refund/fulfillment warnings.';
