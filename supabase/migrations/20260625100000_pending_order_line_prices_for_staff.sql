-- Packing workers need order-line prices visible while preparing shipments.
-- Keep sensitive admin-only accounting fields gated, but expose the line sale
-- values to active inventory staff through the fast pending-order queue RPC.
...
comment on function public.list_pending_ebay_order_queue(text, boolean, integer, integer)
  is 'Lightweight pending eBay order queue read path. Active inventory staff can see line prices for packing; admin-only accounting fields remain gated.';
