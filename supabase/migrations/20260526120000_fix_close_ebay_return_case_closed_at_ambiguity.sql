...
begin
  ...
  where (v_return_id is not null and c.ebay_return_id = v_return_id)
     or (v_order_number is not null and c.order_number = v_order_number and (v_buyer_username is null or c.buyer_username = v_buyer_username))
  ...
  update public.ebay_return_cases as c
  set status = 'closed',
      closed_at = coalesce(c.closed_at, v_closed_at),
      raw_payload = coalesce(c.raw_payload, '{}'::jsonb)
        || v_payload
        || jsonb_build_object(
          'returnStatus', 'CLOSED',
          'returnState', 'CLOSED',
  ...
  with resolved as (
    ...
    where t.return_case_id = v_case.id
      and t.task_type in ('return_review', 'return_intake')
