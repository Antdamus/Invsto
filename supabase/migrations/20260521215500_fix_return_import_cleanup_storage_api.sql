...
-- Replace the test-data cleanup RPC so it reports Storage paths to the app,
-- and the app deletes those objects through the Storage API before removing
-- the return records.

drop function if exists public.admin_clear_ebay_return_import_test_data(boolean);
...
begin
  ...
  complaint_storage_objects := cardinality(v_storage_names);
  complaint_storage_paths := v_storage_names;
