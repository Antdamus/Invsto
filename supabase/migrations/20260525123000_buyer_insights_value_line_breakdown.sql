-- Line-level support for Buyer Insight drill-down cards.
-- This keeps the main modal compact while allowing each summary value to show
-- the exact order/item rows that contribute to it.
  ...
  line_rows as (
    select
      ...
      case
        ...
        when rl.order_status = 'archived' or rl.line_status = 'skipped' then 'archived'
        else 'unknown'
      end as item_state,
      case
        when rabo.order_number is not null and lot.line_total_sum > 0
          then round(rabo.returned_amount * (rl.line_total / lot.line_total_sum), 2)
        when rabo.order_number is not null
