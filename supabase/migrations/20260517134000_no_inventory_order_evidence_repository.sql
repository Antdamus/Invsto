-- Permanent evidence photo repository for no-inventory eBay order completions.
-- Camera captures still land in the temporary capture area first; only photos
-- attached at final confirmation are copied into this bucket and audit payload.

insert into storage.buckets (id, name, public)
values ('order-evidence-photos', 'order-evidence-photos', false)
on conflict (id) do nothing;

drop policy if exists "Inventory staff upload order evidence photos" on storage.objects;
create policy "Inventory staff upload order evidence photos"
on storage.objects
for insert
to authenticated
with check (bucket_id = 'order-evidence-photos' and public.can_manage_inventory());

drop policy if exists "Inventory staff read order evidence photos" on storage.objects;
create policy "Inventory staff read order evidence photos"
on storage.objects
for select
to authenticated
using (bucket_id = 'order-evidence-photos' and public.can_manage_inventory());

drop policy if exists "Admins delete order evidence photos" on storage.objects;
create policy "Admins delete order evidence photos"
on storage.objects
for delete
to authenticated
using (bucket_id = 'order-evidence-photos' and public.is_admin());

create or replace function public.complete_ebay_order_lines_without_inventory_evidence(
...
revoke all on function public.complete_ebay_order_lines_without_inventory_evidence(
  uuid[], text, text, uuid, numeric, numeric, numeric, timestamptz, text, jsonb
) from public;

grant execute on function public.complete_ebay_order_lines_without_inventory_evidence(
  uuid[], text, text, uuid, numeric, numeric, numeric, timestamptz, text, jsonb
) to authenticated;
