...
qs("#udResendBtn")?.addEventListener("click", async () => {
  ...
  try {
    const res = await window.resendInvite({ employee_id: activeEmployeeId, email });

    // ✅ Better message based on backend response
    const method = String(res?.method || "invite");
    msg.textContent = method === "recovery"
      ? "Password setup link sent 🔐"
      : "Invite resent ✉️";

    // ✅ Close the drawer after a short beat so user sees success
    setTimeout(() => {
      closeUserDrawer();
    }, 450);
