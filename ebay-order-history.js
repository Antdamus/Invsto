const state = {
  ...
  relatedReturnEvents: [],
  filteredLines: [],
  historyLineSearchIndex: new Map(),
...
function normalizeLine(row) {
  ...
  const financePayload = getFinancePayload(row);
  return {
    ...row,
    order,
    searchText: buildHistorySearchText([
      ...
      apiSourceLabel,
      financePayload?.statusLabel,
      financePayload?.status,
      labelSearchText,
    ]),
  };
}
...
function getFinanceStatusBadge(line = {}) {
  ...
  const parts = [
    ...
    payload?.syncedAt ? `Checked ${formatDateTime(payload.syncedAt)}` : "",
  ].filter(Boolean);
  return {
    label,
...
function setupDefaultDates() {
  ...
  const weekStart = toDateInputValue(getMondayOfCurrentWeek(todayDate));
  if ($("history-from") && !$("history-from").value) $("history-from").value = weekStart;
...
function applyOrderHistoryDateRange(query, options = {}) {
  let nextQuery = query;
  if (options.fromIso) nextQuery = nextQuery.gte("fulfilled_at", options.fromIso);
  if (options.toIso) nextQuery = nextQuery.lte("fulfilled_at", options.toIso);
  return nextQuery;
}

async function fetchOrderHistoryLinesByIds(lineIds = [], options = {}) {
  ...
  for (const chunk of chunkArray(cleanLineIds, ORDER_HISTORY_RELATED_ID_CHUNK_SIZE)) {
    let query = supabase
      ...
      .in("line_status", ["fulfilled", "cancelled", "skipped"])
      .in("id", chunk);
    query = applyOrderHistoryDateRange(query, options);
    const { data, error } = await query.limit(ORDER_HISTORY_PAGE_SIZE);
...
async function fetchOrderHistoryLinesByOrderIds(orderIds = [], options = {}) {
...
async function expandOrderHistoryRowsByRelatedEvents(rows = [], searchTerm = "", options = {}) {
  ...
  const shouldSearchTrackingEvents = historySearchLooksLikeLabelScan(searchTerm);
  if (!baseLineIds.length && !baseOrderIds.length && !shouldSearchTrackingEvents) {
  ...
  const [linesByIdResult, linesByOrderResult] = await Promise.all([
    fetchOrderHistoryLinesByIds(missingLineIds, options),
    fetchOrderHistoryLinesByOrderIds(missingOrderIds, options),
  ]);
...
async function fetchTargetedRangeOrderHistoryLines(searchTerm, fromIso, toIso) {
  const cleanTerm = String(searchTerm || "").trim();
  const rangeOptions = { fromIso, toIso };
  if (!cleanTerm) return { data: [], error: null, capped: false, relatedLineIds: [] };

  const [
    labelOrderIds,
    orderNumberIds,
    returnTrackingOrderIds,
    labelTrackerEventsResult,
  ] = await Promise.all([
    fetchOrderIdsByLabelMetadataSearch(cleanTerm),
    fetchOrderIdsByOrderNumberSearch(cleanTerm),
    fetchOrderIdsByReturnTrackingSearch(cleanTerm),
    fetchLabelEventsByTrackingSearch(cleanTerm),
  ]);

  if (labelTrackerEventsResult.error) {
    console.warn("Could not search label events by tracking number:", labelTrackerEventsResult.error);
  }

  const labelTrackerEvents = labelTrackerEventsResult.data || [];
  const eventLineIds = [];
  const eventOrderIds = [];
  const eventOrderNumbers = [];
  labelTrackerEvents.forEach((event) => {
    eventLineIds.push(...(event.order_line_ids || []));
    eventOrderIds.push(...(event.order_ids || []));
    eventOrderNumbers.push(...getEventCoveredOrderNumbers(event));
  });

  const eventOrderNumberIds = await fetchOrderIdsByOrderNumbers(eventOrderNumbers);
  const orderIds = [...new Set([
    ...labelOrderIds,
    ...orderNumberIds,
    ...returnTrackingOrderIds,
  ...
  const [rowsByOrderResult, rowsByIdResult] = await Promise.all([
    fetchOrderHistoryLinesByOrderIds(orderIds, rangeOptions),
    fetchOrderHistoryLinesByIds(eventLineIds, rangeOptions),
  ]);

  if (rowsByOrderResult.error) return { data: rowsByOrderResult.data || [], error: rowsByOrderResult.error };
  if (rowsByIdResult.error) return { data: rowsByIdResult.data || [], error: rowsByIdResult.error };

  const rows = uniqueById([
    ...(rowsByOrderResult.data || []),
    ...(rowsByIdResult.data || []),
  ]);
  const expanded = await expandOrderHistoryRowsByRelatedEvents(rows, cleanTerm, rangeOptions);
  const expandedRows = uniqueById(expanded.rows || rows);
  ...
  return {
    ...
    error: null,
    capped: expandedRows.length >= ORDER_HISTORY_MAX_LINES,
    relatedLineIds: expanded.relatedLineIds || [],
  };
}

async function fetchAllDateOrderHistoryLines(searchTerm) {
  const buyerTerm = String(searchTerm || "").trim();
  if (!buyerTerm) return { data: [], error: null, capped: false, relatedLineIds: [] };

  const searchLooksLikeTracking = historySearchLooksLikeLabelScan(buyerTerm);
  ...
  const [
    ...
    fetchOrderIdsByLabelMetadataSearch(buyerTerm),
    fetchOrderIdsByOrderNumberSearch(buyerTerm),
    fetchOrderIdsByReturnTrackingSearch(buyerTerm),
    fetchLabelEventsByTrackingSearch(buyerTerm),
  ]);
  ...
  if (!orderIds.length && !eventLineIds.length) return { data: [], error: null, capped: false, relatedLineIds: [] };
  ...
  const rowsByIdResult = await fetchOrderHistoryLinesByIds(eventLineIds);
  if (rowsByIdResult.error) return { data: rows, error: rowsByIdResult.error };
  rows.push(...(rowsByIdResult.data || []));

  const expanded = await expandOrderHistoryRowsByRelatedEvents(rows, buyerTerm);
  const expandedRows = uniqueById(expanded.rows || rows);
  expandedRows.sort((a, b) => new Date(b.fulfilled_at || 0) - new Date(a.fulfilled_at || 0));
...
async function loadOrderHistory() {
  ...
  const searchAllDates = Boolean(searchTerm && buyerAllDates);
  const targetedRangeSearch = Boolean(searchTerm && !searchAllDates && historySearchLooksLikeLabelScan(searchTerm));
  if ((searchAllDates || targetedRangeSearch) && list) {
    const scope = searchAllDates ? "all stored history" : "selected date range";
    const searchKind = historySearchLooksLikeLabelScan(searchTerm) ? "label/tracking scan" : "buyer/order search";
    list.innerHTML = `<div class="history-empty">Searching ${escapeHtml(scope)} for ${escapeHtml(searchKind)} "${escapeHtml(searchTerm)}"...</div>`;
  }

  const linesQuery = searchAllDates
    ? fetchAllDateOrderHistoryLines(searchTerm)
    : targetedRangeSearch
    ? fetchTargetedRangeOrderHistoryLines(searchTerm, fromIso, toIso)
  ...
  if (linesResult.capped) {
    console.warn(
      searchAllDates
        ? `Order History loaded the first ${ORDER_HISTORY_MAX_LINES} closed lines for all-date search "${searchTerm}". Use a more exact buyer, order, or tracking value if you need fewer rows.`
        : targetedRangeSearch
        ? `Order History loaded the first ${ORDER_HISTORY_MAX_LINES} matching closed lines for label/tracking scan "${searchTerm}" in this date range. Narrow the dates if you need fewer rows.`
  ...
  if (closedLineIds.length) {
    const [relatedAdminResult, relatedRevertResult, relatedLabelResult, returnCasesResult, relatedReturnResult, relatedOrderTaskResult] = await Promise.all([
      ...
      loadOrderTaskDataForLines(closedLineIds, closedOrderIds, { lines: closedLines })
        .then((data) => ({ data, error: null }))
        .catch((error) => ({ data: { tasks: [], events: [] }, error })),
    ]);
    ...
    if (relatedOrderTaskResult.error) {
      console.warn("Failed to load order coordination tasks for visible order lines:", relatedOrderTaskResult.error);
    } else {
      relatedOrderTasks = relatedOrderTaskResult.data?.tasks || [];
      relatedOrderTaskEvents = relatedOrderTaskResult.data?.events || [];
    }
  }
  ...
  state.relatedOrderTasks = relatedOrderTasks;
  ...
  state.historyLoadedTargetedSearch = targetedRangeSearch;
  state.historyLoadedAllDatesSearch = searchAllDates;
...
const ORDER_HISTORY_TASK_SELECT = "id, order_id, order_line_ids, parent_task_id, task_type, title, question, status, priority, assigned_to_user_id, assigned_to_email, assigned_by_email, due_at, latest_note, latest_photo_count, created_by_email, created_at, updated_at, metadata";

async function loadOrderTaskDataForLines(lineIds = [], orderIds = [], options = {}) {
  const cleanLineIds = [...new Set(lineIds.filter(Boolean))];
  const cleanOrderIds = [...new Set(orderIds.filter(Boolean))];
  if (!cleanLineIds.length && !cleanOrderIds.length) return { tasks: [], events: [] };
  ...
  const tasks = uniqueById(taskRows).map((task) => {
    ...
    if (taskLineIds.length || !task.order_id) return task;
    return {
      ...task,
  ...
  return { tasks, events: enrichedEvents };
}

async function loadOrderTaskEventsForLines(lineIds = []) {
  const result = await loadOrderTaskDataForLines(lineIds, [], { lines: state.lines });
...
function getHistoryLineSearchHaystack(line) {
  if (!line) return "";
  return [
    line.searchText,
...
function rebuildHistoryLineSearchIndex() {
  state.historyLineSearchIndex = new Map(
    state.lines.map((line) => [line.id, getHistoryLineSearchHaystack(line)])
  );
}

function getIndexedHistoryLineSearchHaystack(line) {
  if (!line?.id) return getHistoryLineSearchHaystack(line);
  return state.historyLineSearchIndex.get(line.id) || getHistoryLineSearchHaystack(line);
}

function loadedHistoryHasLocalSearchMatch(term) {
...
function shouldReloadHistoryForSearchInput(value) {
  ...
  const buyerAllDates = Boolean($("history-buyer-all-dates")?.checked);
  if (!cleanTerm) return state.historyLoadedTargetedSearch || state.historyLoadedAllDatesSearch;
  if (buyerAllDates) return true;
  if (!historySearchLooksLikeLabelScan(cleanTerm)) {
    return state.historyLoadedTargetedSearch || state.historyLoadedAllDatesSearch;
  }
  if (!state.historyLoaded || !state.lines.length) return true;
...
function getReturnTaskEventPayload(event = {}) {
  return event.payload && typeof event.payload === "object" ? event.payload : {};
}

function isSystemReturnTaskNoteText(note = "") {
  return /return intake task opened from ebay return api|return review task opened from ebay return api|task opened from ebay return api|created from ebay return api|synced from ebay return api/i
    .test(String(note || ""));
}

function isVisibleReturnTaskUpdateEvent(event = {}) {
  const note = String(event.notes || "").trim();
  if (!note) return false;
  if (event.action === "created") return false;
  if (isSystemReturnTaskNoteText(note)) return false;
  const payload = getReturnTaskEventPayload(event);
  if (/(api|import|sync)/i.test(String(payload.source || "")) && !event.signed_by_email) return false;
  return ["assigned", "cancelled", "commented", "resolved", "status_changed"].includes(event.action)
    || Boolean(event.signed_by_email);
}

function getReturnTaskUpdateEvents(task = {}, limit = 6) {
  const events = getReturnTaskEvents(task)
    .filter(isVisibleReturnTaskUpdateEvent)
    .slice(0, limit);
  if (!events.length && String(task.latest_note || "").trim() && !isSystemReturnTaskNoteText(task.latest_note)) {
    return [{
...
function renderReturnTaskSummary(task = {}) {
  ...
  const financePill = getReturnTaskFinancePill(task);
    ...
    <div class="return-task-expanded">
      <div class="return-task-main">
        ...
        <div class="return-task-buttons">
          ...
          ${isAdminUser() ? `<button type="button" class="secondary-btn" data-return-task-assign="${escapeHtml(task.id)}">Assign Task</button>` : ""}
...
function getReturnQueueCountLabel(visibleCount = 0, totalCount = 0) {
  const statusFilter = $("return-task-status-filter")?.value || "pending";
  const issueFilter = $("return-task-issue-filter")?.value || "all";
  const assigneeFilter = $("return-task-assignee-filter")?.value || "";
...
function getReturnAssigneePromptText() {
  return state.returnAssignees
    .map((employee, index) => `${index + 1}. ${employee.display_name || employee.email || "Worker"}${employee.email ? ` (${employee.email})` : ""}`)
    .join("\n");
}

function resolveReturnAssigneeChoice(value = "") {
  const clean = String(value || "").trim();
  if (!clean) return null;
  const index = Number(clean);
  if (Number.isInteger(index) && index >= 1 && index <= state.returnAssignees.length) {
    return state.returnAssignees[index - 1];
  }
  const normalized = clean.toLowerCase();
  return state.returnAssignees.find((employee) => [
    employee.display_name,
    employee.email,
    employee.user_id,
  ].filter(Boolean).some((entry) => String(entry).toLowerCase() === normalized || String(entry).toLowerCase().includes(normalized))) || null;
}
...
async function assignReturnTask(taskId, button = null) {
  ...
  if (!task) return;
  if (!state.returnAssignees.length) await loadReturnAssignees();
  const promptText = getReturnAssigneePromptText();
  if (!promptText) {
    setReturnTaskSaveStatus("No active assignees are available.", "error");
    return;
  }
  const answer = window.prompt(
    `Assign this return task to:\n\n${promptText}\n\nType the number, name, or email.`,
    ""
  );
  if (answer === null) return;
  const assignee = resolveReturnAssigneeChoice(answer);
  if (!assignee?.user_id) {
    setReturnTaskSaveStatus("Could not match that assignee. Use the number, exact name, or email.", "error");
    return;
  }
  ...
  try {
    const { error } = await supabase.rpc("assign_ebay_return_task", {
      _task_id: taskId,
      _assigned_to_user_id: assignee.user_id,
      _priority: priority,
      _due_at: dueAt,
      _notes: `Task assigned to ${assignee.display_name || assignee.email || "worker"}.`,
      _signed_by_email: state.user?.email || null,
    });
    if (error) throw error;
    await loadReturnQueue();
    const assigneeLabel = getReturnAssigneeLabel(assignee.user_id);
    ...
    if (button) {
      ...
      button.textContent = originalText || "Assign Task";
    }
  }
}
...
function summarizeHistoryOrderTask(task = {}) {
  ...
  return {
    ...task,
    status: task.status || latestEvent?.new_status || "",
    title: task.title || latestEvent?.task_title || "",
    question: task.question || latestEvent?.task_question || "",
    assignee: task.assigned_to_email || latestEvent?.task_assigned_to_email || "",
    dueAt: task.due_at || latestEvent?.task_due_at || "",
    latestAt: task.updated_at || latestEvent?.created_at || task.created_at || "",
    latestEvent,
    events,
  };
}
...
function renderHistoryOrderTaskLineSelect(option = getSelectedHistoryOrderTaskOption()) {
  const scope = getHistoryOrderTaskScopeValue();
  const field = $("history-order-task-line-field");
  const select = $("history-order-task-line");
  if (!field || !select) return;
  const lines = Array.isArray(option?.lines) ? option.lines : [];
  const currentValue = select.value || "";
  field.classList.toggle("hidden", scope !== "line");
  select.replaceChildren();
  lines.forEach((line, index) => {
    const label = [
      line.item_number || `Line ${index + 1}`,
      line.item_title || "Untitled item",
      `Qty ${Number(line.quantity || 1).toLocaleString()}`,
      formatMoney(getLineGross(line)),
    ].filter(Boolean).join(" - ");
    select.appendChild(new Option(label, line.id || ""));
  });
  if ([...select.options].some((entry) => entry.value === currentValue)) {
    select.value = currentValue;
  } else if (lines[0]) {
    select.value = lines[0].id || "";
  }
}
...
async function uploadHistoryOrderTaskPhotos(files, option = {}, scopedLines = option.lines || []) {
  ...
  const cleanLineIds = [...new Set((scopedLines || []).map((line) => line.id).filter(Boolean))];
  const scope = getHistoryOrderTaskScopeValue();
  const uploaded = [];
...
function getHistoryTaskAssigneeLabel(task = {}) {
  return task.assignee || task.assigned_to_email || task.assigned_by_email || task.created_by_email || "Unassigned";
}

function getHistoryTaskLatestUpdateText(task = {}) {
  const event = task.latestEvent || null;
  return getDisplayHistoryNote(event?.notes || task.latest_note || task.question || "No instruction recorded.");
}

function getHistoryTaskLatestUpdateActor(task = {}) {
  const event = task.latestEvent || null;
  return event?.signed_by_email || event?.created_by_email || task.created_by_email || task.assigned_by_email || "Unknown user";
}

function getHistoryTaskOpenUrl(task = {}) {
  return task.id ? `team-tasks.html?taskId=${encodeURIComponent(task.id)}` : "";
}

function getHistoryTaskEvidenceCount(task = {}) {
  const eventPhotos = (task.events || []).reduce((sum, event) => sum + getEventEvidencePhotos(event).length, 0);
  return Math.max(Number(task.latest_photo_count || 0), eventPhotos);
}

function renderHistoryOrderTaskCard(task = {}, options = {}) {
  const active = isActiveHistoryOrderTaskStatus(task.status);
  const compact = options.compact === true;
  const title = task.title || "Closed order task";
  const updateText = getHistoryTaskLatestUpdateText(task);
  const openUrl = getHistoryTaskOpenUrl(task);
  const evidenceCount = getHistoryTaskEvidenceCount(task);
  return `
    <article class="history-order-task-card ${active ? "is-active" : "is-closed"} priority-${escapeHtml(task.priority || "normal")} ${compact ? "is-compact" : ""}">
      <div>
        <div class="history-order-task-card-head">
          <span class="history-status ${active ? "is-admin" : ""}">${escapeHtml(getHistoryOrderTaskStatusLabel(task.status))}</span>
          <strong>${escapeHtml(title)}</strong>
        </div>
        ...
        <div class="history-order-task-card-meta">
          ...
          ${evidenceCount ? `<span>${evidenceCount.toLocaleString()} media</span>` : ""}
        </div>
        ${!compact && updateText ? `
          <small class="history-order-task-latest">${escapeHtml(formatDateTime(task.latestAt))} - ${escapeHtml(getHistoryTaskLatestUpdateActor(task))} - ${escapeHtml(updateText)}</small>
        ` : ""}
      </div>
      ${openUrl ? `<a class="secondary-btn history-task-open-link" href="${escapeHtml(openUrl)}">Open task</a>` : ""}
    </article>
  `;
}

function renderHistoryOrderTaskStrip(group = {}) {
  const taskSummary = getHistoryGroupTaskSummary(group);
  if (!taskSummary.totalCount) return "";
  const tasks = taskSummary.tasks.slice(0, 8);
  const hiddenCount = Math.max(0, taskSummary.totalCount - tasks.length);
  return `
    <section class="history-order-task-strip">
      <div class="history-order-task-strip-head">
        <div>
          <span class="eyebrow">Assigned Tasks</span>
          <strong>${taskSummary.activeCount ? `${taskSummary.activeCount} active` : "No active tasks"}</strong>
          <small>${taskSummary.totalCount.toLocaleString()} total task${taskSummary.totalCount === 1 ? "" : "s"} tied to this closed order group</small>
        </div>
        <button type="button" class="secondary-btn history-task-btn" data-history-order-task="${escapeHtml(getHistoryGroupKey(group))}">Add Task</button>
      </div>
      <div class="history-order-task-list">
        ${tasks.map((task) => renderHistoryOrderTaskCard(task)).join("")}
        ${hiddenCount ? `<div class="history-order-task-more">${hiddenCount.toLocaleString()} more task${hiddenCount === 1 ? "" : "s"}</div>` : ""}
      </div>
    </section>
  `;
}
...
function renderHistoryLineTaskStrip(line = {}) {
  ...
  return `
    <div class="history-line-task-strip">
      <div class="history-line-task-head">
        <span>Assigned task${taskSummary.totalCount === 1 ? "" : "s"}</span>
        <strong>${taskSummary.activeCount ? `${taskSummary.activeCount} active` : `${taskSummary.totalCount} total`}</strong>
      </div>
      <div class="history-line-task-list">
        ${tasks.map((task) => renderHistoryOrderTaskCard(task, { compact: true })).join("")}
      </div>
    </div>
  `;
}
  ...
  groups.forEach((group, groupIndex) => {
    ...
    const detailHtml = isExpanded ? `
      <div class="history-order-detail">
        ...
        ${renderHistoryOrderTaskStrip(group)}
        ${groupPhotos.length ? `
          ...
          ${group.lines.map((line) => {
            ...
            <div class="history-line-row ${lineIssue ? "has-return-issue" : ""} ${lineIssue?.targeted ? "is-target-return-line" : ""}">
              <div>
                ...
                ${renderFinanceBadgeMarkup(lineFinanceStatus, "history-line-finance-pill")}
                ${renderHistoryLineReturnIssue(line)}
                ${renderHistoryLineTaskStrip(line)}
    ...
    card.querySelectorAll("[data-history-order-task]").forEach((button) => {
...
function setupListeners() {
  ...
  $("history-buyer-all-dates")?.addEventListener("change", () => {
    window.clearTimeout(historyTargetedSearchTimer);
    const value = String($("history-search")?.value || "").trim();
    if (value || state.historyLoadedAllDatesSearch) {
      loadOrderHistory();
      return;
    }
    applyFilters();
  ...
  $("history-order-task-scope")?.addEventListener("change", renderHistoryOrderTaskContext);
  $("history-order-task-line")?.addEventListener("change", renderHistoryOrderTaskContext);
