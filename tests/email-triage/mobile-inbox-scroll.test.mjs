...
test("mobile inbox switcher uses restore mode and ships with a fresh cache key", () => {
  ...
  assert.match(html, /email-triage\.js\?v=task-history-related-set-20260823/);
});

test("mobile eBay workspace stays compact and touch-friendly on phones", () => {
  assert.match(html, /email-triage\.css\?v=task-history-related-set-20260823/);
...
test("mobile task composer renders as a compact phone sheet", () => {
  assert.match(html, /email-triage-asset-version" content="task-history-related-set-20260823"/);
